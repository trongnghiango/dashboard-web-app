...
const ModalUpdateNftRarity = ({
  ...
  const submit = (values: NftRarity, { setSubmitting }: FormikHelpers<NftRarity>) => {
    updateData(values);
  ...
  const updateData = (values: NftRarity) => {
    // console.log("---------params", params);
    setLoading(true)(dispatch);
    const token = getUserToken(false);
    const id = data.id;

    const params: UpdateNftRarityInput = {
      name: values.name,
      color: values.color,
    ...
    NftRarityService.update({ id, data: params, token })
      .then(() => {
        loadData();
  ...
  const getInitialValues = (params: NftRarity) => {
    const data: NftRarity = {
...
export default ModalUpdateNftRarity;
