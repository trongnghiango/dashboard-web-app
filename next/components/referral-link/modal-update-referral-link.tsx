...
const ModalUpdateReferralLink = ({
  ...
  const updateData = (values: ReferralLink) => {
    // console.log("---------params", params);
    setLoading(true)(dispatch);
    const token = getUserToken(false);
    const id = data.id;

    const params: UpdateReferralLinkInput = {
      name: values.name,
    };

    ReferralLinkService.update({ id, data: params, token })
      .then(() => {
        loadData();
  ...
  const getInitialValues = (params: ReferralLink) => {
      ...
      >
        {({ errors, touched, values, setFieldValue }) => (
          <Form>
            <div className="p-3">
              <InputField
                labelName={ReferralLinkArgNames.name}
                inputName={ReferralLinkArgs.name}
                placeholder={`Input ${ReferralLinkArgNames.name}`}
...
export default ModalUpdateReferralLink;
