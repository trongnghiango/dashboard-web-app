...
const ModalCreateReferralLink = ({
  ...
  const submit = (
    ...
    { setSubmitting }: FormikHelpers<CreateReferralLinkInput>
  ) => {
    createData({...values});
  ...
  const createData = (data: CreateReferralLinkInput) => {
    setLoading(true)(dispatch);
    const token = getUserToken(false);
    ReferralLinkService.create({ data, token })
      .then(() => {
        loadData();
  ...
  return (
    <Dialog
      ...
      mobileMode={false}
      title="Create new referralLink"
    >
      <Formik
        initialValues={ReferralLinkInitialValues}
