...
const ModalUpdateNftElement = ({
  ...
  const submit = (values: NftElement, { setSubmitting }: FormikHelpers<NftElement>) => {
    updateData(values);
  ...
  const updateData = (values: NftElement) => {
    // console.log("---------params", params);
    setLoading(true)(dispatch);
    const token = getUserToken(false);
    const id = data.id;

    const params: UpdateNftElementInput = {
      name: values.name,
      color: values.color,
    ...
    NftElementService.update({ id, data: params, token })
      .then(() => {
        loadData();
  ...
  const getInitialValues = (params: NftElement) => {
    const data: NftElement = {
...
export default ModalUpdateNftElement;
