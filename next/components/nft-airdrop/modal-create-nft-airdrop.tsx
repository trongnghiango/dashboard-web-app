...
import {
  NftAirdropArgNames,
  NftAirdropArgs,
  NftAirdropInitialValues,
  CreateNftAirdropInput,
...
const ModalCreateNftAirdrop = ({
  ...
  const submit = (
    values: CreateNftAirdropInput,
    { setSubmitting }: FormikHelpers<CreateNftAirdropInput>
  ) => {
    createData({ ...values });
  ...
  const createData = (data: CreateNftAirdropInput) => {
    setLoading(true)(dispatch);
    const token = getUserToken(false);
    NftAirdropService.create({ data, token })
      .then(() => {
        loadData();
  ...
  return (
    <Dialog
      ...
      mobileMode={false}
      title="Create new nftAirdrop"
    >
      <Formik
        initialValues={NftAirdropInitialValues}
...
export default ModalCreateNftAirdrop;
