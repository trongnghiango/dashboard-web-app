...
import ReferralLinkTableList from "../components/referral-link/referral-link-table-list";
import ReferralLinkActionCreate from "../components/referral-link/referral-link-action-create";

const ReferralLinkPage = (props: any) => {
  useAuth(ROLES.ADMIN_EDITOR);
  
  const [page, setPage] = useState(1);
  const [data, setData] = useState<ReferralLink[]>();
  ...
  const getData = async ({
    ...
    ReferralLinkService.getAll({
      ...
      token,
      cache: false,
      fragment: ReferralLinkService.fullFragment,
    })
      .then(handleResult)
      .catch(handleError);
  };
...
ReferralLinkPage.Layout = AdminLayout;
  ...
  return (
    <PageHeader>
      ...
      <PageHeaderFunctions setViewMode={setViewMode}>
        ...
        <ReferralLinkActionCreate loadData={loadData} />
      </PageHeaderFunctions>
    </PageHeader>
  );
});
