...
import NftRarityTableList from "../components/nft-rarity/nft-rarity-table-list";
import NftRarityActionCreate from "../components/nft-rarity/nft-rarity-action-create";

const NftRarityPage = (props: any) => {
  useAuth(ROLES.ADMIN_EDITOR);
  
  const [page, setPage] = useState(1);
  const [data, setData] = useState<NftRarity[]>(null);
  ...
  const getData = async ({
    ...
    NftRarityService.getAll({
      ...
      token,
      cache: false,
      fragment: NftRarityService.fullFragment,
    })
      .then(handleResult)
      .catch(handleError);
  };
...
NftRarityPage.Layout = AdminLayout;
export default NftRarityPage;
  ...
  return (
    <PageHeader>
      ...
      <PageHeaderFunctions setViewMode={setViewMode}>
        ...
        <NftRarityActionCreate loadData={loadData} />
      </PageHeaderFunctions>
    </PageHeader>
  );
});
