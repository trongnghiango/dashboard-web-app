...
import { ReferralTransactionService } from "../lib/modules/referral-transaction/referral-transaction.repo";
import ReferralTransactionTableList from "../components/referral-transaction/referral-transaction-table-list";

const ReferralTransactionPage = (props: any) => {
  useAuth(ROLES.ADMIN_EDITOR);
  
  const [page, setPage] = useState(1);
  const [data, setData] = useState<ReferralTransaction[]>();
  ...
  const getData = async ({
    ...
    ReferralTransactionService.getAll({
      query: {
        limit: length,
      ...
      token,
      cache: false,
      fragment: ReferralTransactionService.fullFragment,
    })
      .then(handleResult)
      .catch(handleError);
  };
...
ReferralTransactionPage.Layout = AdminLayout;
