...
import TypeEventTableList from "../components/type-event/type-event-table-list";
import TypeEventActionCreate from "../components/type-event/type-event-action-create";

const TypeEventPage = (props: any) => {
  useAuth(ROLES.ADMIN_EDITOR);

  const [page, setPage] = useState(1);
  const [data, setData] = useState<TypeEvent[]>();
  ...
  const getData = async ({ page, length }: { page: number; length: number }) => {
    ...
    TypeEventService.getAll({
      ...
      token,
      cache: false,
      fragment: TypeEventService.fullFragment,
    })
      .then(handleResult)
      .catch(handleError);
  };
...
TypeEventPage.Layout = AdminLayout;
  ...
  return (
    <PageHeader>
      ...
      <PageHeaderFunctions setViewMode={setViewMode}>
        ...
        <TypeEventActionCreate loadData={loadData} />
      </PageHeaderFunctions>
    </PageHeader>
  );
});
...
const Table = ({
  ...
  return (
    <div className="bg-seconds-light pb-1">
      <TableListWrapper>
        <TypeEventTableList data={data} loadData={loadData} viewMode={viewMode} />
      </TableListWrapper>
      <Paging
