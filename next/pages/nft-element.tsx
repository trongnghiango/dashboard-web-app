...
import NftElementTableList from "../components/nft-element/nft-element-table-list";
import NftElementActionCreate from "../components/nft-element/nft-element-action-create";

const NftElementPage = (props: any) => {
  useAuth(ROLES.ADMIN_EDITOR);
  
  const [page, setPage] = useState(1);
  const [data, setData] = useState<NftElement[]>();
  ...
  const getData = async ({
    ...
    NftElementService.getAll({
      ...
      token,
      cache: false,
      fragment: NftElementService.fullFragment,
    })
      .then(handleResult)
      .catch(handleError);
  };
...
NftElementPage.Layout = AdminLayout;
export default NftElementPage;
  ...
  return (
    <PageHeader>
      ...
      <PageHeaderFunctions setViewMode={setViewMode}>
        ...
        <NftElementActionCreate loadData={loadData} />
      </PageHeaderFunctions>
    </PageHeader>
  );
});
