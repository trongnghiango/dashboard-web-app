...
import NftAirdropTableList from "../components/nft-airdrop/nft-airdrop-table-list";
import NftAirdropActionCreate from "../components/nft-airdrop/nft-airdrop-action-create";

const NftAirdropPage = (props: any) => {
  useAuth(ROLES.ADMIN_EDITOR);

  const [page, setPage] = useState(1);
  const [data, setData] = useState<NftAirdrop[]>(null);
  ...
  const getData = async ({ page, length }: { page: number; length: number }) => {
    ...
    NftAirdropService.getAll({
      ...
      token,
      cache: false,
      fragment: NftAirdropService.fullFragment,
    })
      .then(handleResult)
      .catch(handleError);
  };
...
NftAirdropPage.Layout = AdminLayout;
  ...
  return (
    <PageHeader>
      ...
      <PageHeaderFunctions setViewMode={setViewMode}>
        ...
        {user?.role === ROLES.ADMIN && <NftAirdropActionCreate loadData={loadData} />}
      </PageHeaderFunctions>
    </PageHeader>
  );
});
...
const Table = ({
  ...
  return (
    <div className="bg-seconds-light pb-1">
      <TableListWrapper>
        <NftAirdropTableList data={data} loadData={loadData} viewMode={viewMode} />
      </TableListWrapper>
      <Paging
