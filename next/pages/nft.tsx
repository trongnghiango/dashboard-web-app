...
import NftTableList from "../components/nft/nft-table-list";
import NftActionCreate from "../components/nft/nft-action-create";
...
const NftPage = (props: any) => {
  ...
  const getData = async ({ page, length }: { page: number; length: number }) => {
    ...
    NftService.getAll({
      ...
      token,
      cache: false,
      fragment: NftService.fullFragment,
    })
      .then(handleResult)
      .catch(handleError);
  };
  ...
  return (
    <PageHeader>
      ...
      <PageHeaderFunctions setViewMode={setViewMode}>
        ...
        {user?.role === ROLES.ADMIN && currentAccount && <NftActionCreate loadData={loadData} />}
      </PageHeaderFunctions>
      {isOpen && (
...
const Table = ({
  ...
  return (
    <div className="bg-seconds-light pb-1">
      <TableListWrapper>
        <NftTableList data={data} loadData={loadData} viewMode={viewMode} />
      </TableListWrapper>
      <Paging
