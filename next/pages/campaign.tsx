...
import { getUserToken, ROLES, UserRole } from "../lib/modules/user/user.model";
...
import CampaignTableList from "../components/campaign/campaign-table-list";
import CampaignActionCreate from "../components/campaign/campaign-action-create";
...
const CampaignPage = (props: any) => {
  ...
  const getData = async ({ page, length }: { page: number; length: number }) => {
    ...
    CampaignService.getAll({
      ...
      token,
      cache: false,
      fragment: CampaignService.fullFragment,
    })
      .then(handleResult)
      .catch(handleError);
  };
  ...
  const loadData = () => {
    getData({ page, length }).then((result:any) => {});
  };
...
CampaignPage.Layout = AdminLayout;
  ...
  return (
    <PageHeader>
      ...
      <PageHeaderFunctions setViewMode={setViewMode}>
        ...
        {user?.role === UserRole.ADMIN && <CampaignActionCreate loadData={loadData} />}
      </PageHeaderFunctions>
    </PageHeader>
  );
});
...
const Table = ({
  ...
  return (
    <div className="bg-seconds-light pb-1">
      <TableListWrapper>
        <CampaignTableList data={data} loadData={loadData} viewMode={viewMode} />
      </TableListWrapper>
      <Paging
