...
import SupportTableList from "../components/support/support-table-list";
...
const SupportPage = (props: any) => {
  ...
  const getData = async ({ page, length }: { page: number; length: number }) => {
    ...
    SupportService.getAll({
      ...
      token,
      cache: false,
      fragment: SupportService.fullFragment,
    })
      .then(handleResult)
      .catch(handleError);
  };
...
const Table = ({
  ...
  return (
    <div className="bg-seconds-light pb-1">
      <TableListWrapper>
        <SupportTableList data={data} loadData={loadData} viewMode={viewMode} />
      </TableListWrapper>
      <Paging
